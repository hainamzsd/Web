    ...
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-purple-600 via-blue-600 to-indigo-700 -m-6 mb-6 p-8 rounded-lg text-white relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-blue-100 text-lg">Hệ thống định danh vị trí toàn quốc</p>
          <div className="flex items-center gap-6 mt-6">
            ...
            <div className="flex items-center gap-2">
              {isGrowing ? <TrendingUp className="h-6 w-6 text-green-300" /> : <TrendingUp className="h-6 w-6 text-red-300 rotate-180" />}
              <span className={`text-2xl font-semibold ${isGrowing ? 'text-green-300' : 'text-red-300'}`}>{monthlyGrowth}%</span>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <div className="bg-white rounded-xl p-6 shadow-lg border-l-4 border-blue-500">
          <div className="flex items-center justify-between">
            ...
            <div className="bg-blue-100 p-3 rounded-lg"><FileText className="h-8 w-8 text-blue-600" /></div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl p-6 shadow-lg border-l-4 border-yellow-500">
          <div className="flex items-center justify-between">
            ...
            <div className="bg-yellow-100 p-3 rounded-lg"><Clock className="h-8 w-8 text-yellow-600" /></div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl p-6 shadow-lg border-l-4 border-green-500">
          <div className="flex items-center justify-between">
            ...
            <div className="bg-green-100 p-3 rounded-lg"><CheckCircle className="h-8 w-8 text-green-600" /></div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl p-6 shadow-lg border-l-4 border-purple-500">
          <div className="flex items-center justify-between">
            ...
            <div className="bg-purple-100 p-3 rounded-lg"><Users className="h-8 w-8 text-purple-600" /></div>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card className="shadow-xl border-0">
          <CardHeader className="bg-gradient-to-r from-blue-50 to-indigo-50">
            <CardTitle className="flex items-center gap-2"><BarChart3 className="h-5 w-5 text-blue-600" />Xu hướng 6 tháng</CardTitle>
          </CardHeader>
        ...
        <Card className="shadow-xl border-0">
          <CardHeader className="bg-gradient-to-r from-purple-50 to-pink-50">
            <CardTitle className="flex items-center gap-2"><MapPin className="h-5 w-5 text-purple-600" />Top 5 tỉnh thành</CardTitle>
          </CardHeader>
          <CardContent className="pt-6">
            <ResponsiveContainer width="100%" height={300}>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className="md:col-span-2 shadow-xl border-0">
          <CardHeader className="bg-gradient-to-r from-gray-50 to-gray-100">
            <CardTitle className="flex items-center gap-2"><Activity className="h-5 w-5 text-gray-700" />Hoạt động gần đây</CardTitle>
          </CardHeader>
          <CardContent className="pt-6">
            <div className="space-y-3">
              {recentActivity.length === 0 ? (
                ...
                recentActivity.map((activity, idx) => (
                  <div key={activity.id} className="flex items-center gap-4 p-3 rounded-lg hover:bg-gray-50 border border-gray-100">
                    <div className="flex-shrink-0 w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center">
                      <span className="text-blue-600 font-bold text-sm">{idx + 1}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-gray-900 truncate">{activity.location_name || 'Chưa đặt tên'}</p>
                    ...
                    <div className="text-right">
                      <div className={`inline-block px-2 py-1 rounded text-xs font-medium ${
                        ...
                        activity.status === 'approved_commune' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-700'
                      }`}>{activity.status}</div>
                      <p className="text-xs text-gray-400 mt-1">{new Date(activity.created_at).toLocaleDateString('vi-VN')}</p>
                    </div>
                  </div>
                ))
              )}
          ...
          <CardContent className="pt-6">
            <div className="space-y-4">
              ...
              <div className="bg-gradient-to-r from-green-500 to-emerald-600 p-5 rounded-xl text-white shadow-lg">
                ...
                <div className="text-3xl font-bold mt-1">{stats.totalSurveys > 0 ? ((stats.published / stats.totalSurveys) * 100).toFixed(1) : 0}%</div>
                <div className="text-xs opacity-75 mt-1">{stats.published}/{stats.totalSurveys} khảo sát</div>
              </div>
